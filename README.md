...
- local `uploads/` access or Supabase Storage configuration

Known production constraints are still intentional for this MVP:

- local uploads need durable private storage or a persistent volume
- Vercel/online deployments should use `FILE_STORAGE_PROVIDER=supabase`
- rate limiting is in-memory and should be backed by Redis or a database for multi-instance deployment

For Vercel + Supabase pilot deployment, configure:
...
Create the Supabase Storage bucket as private. IA Supervisor serves files through authenticated application routes.
...
- Uploaded files use local storage or private Supabase Storage and are served through authenticated `/api/files/[fileId]`
  routes.
...
- Local file storage is the default; online deployment can use private Supabase Storage.
